...
public class ArrayMetric implements Metric {
    ...
    /**
     *
     * @param sampleCount 在一个采集间隔中抽样的个数，默认为2，例如当intervalInMs = 1000时，抽样两次，则一个采集区间中会
     *                    包含两个相等的区间，一个区间就是滑动窗口
     ...
     * @param sampleCount
     * @param intervalInMs
     * @param enableOccupy 是否允许抢占，即当前时间戳已经达到限制后，是否可以占用下一个时间窗口的容量，这里对应LeapArray的两个实现类
     *                     如果允许抢占，则为OccupiableBucketLeapArray,否则为BucketLeapArray
     */
